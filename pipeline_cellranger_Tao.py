'''
Run cellranger on 10X fastq file
'''
...
@transform("data/*/.sample", regex(r'data/(.+)/.sample'), r'\1/outs/filtered_feature_bc_matrix.h5')
def cellranger_count(infile, outfile):
    # python module looking for regular expression, group(1)
    sampleid = re.search('data/(.+)/.sample', infile).group(1)
    print(sampleid)
    fastqs = samples['fastqs'][sampleid]
    cellsnumber = samples['cells'][sampleid]
    ...
    P.run(statement, job_queue='all.q',
          ...
          job_total_memory=params["cellrangercount_memory"],
